...
import { MixedRouteTrade, MixedRouteSDK } from '@x-swap-protocol/router-sdk'
import { Trade as V2Trade, Pair, Route as RouteV2 } from '@x-swap-protocol/v2-sdk'
import { Pool, FeeOptions } from '@x-swap-protocol/v3-sdk'
// import { generatePermitSignature, toInputPermit, makePermit, generateEip2098PermitSignature } from './utils/permit2'
import { CurrencyAmount, Percent, TradeType } from '@x-swap-protocol/sdk-core'
import { registerFixture } from './forge/writeInterop'
import { buildTrade, getXSwapPools, swapOptions, XDC, xUSDT, WXDC, XSP } from './utils/uniswapData'
import { hexToDecimalString } from './utils/hexToDecimalString'
import { TEST_FEE_RECIPIENT_ADDRESS } from './utils/addresses'
// import {
//   PartialClassicQuote,
//   PoolType,
//   RouterTradeAdapter,
...
const FORK_BLOCK = 95721741

// note: these tests aren't testing much but registering calldata to interop file
// for use in forge fork tests
describe('XSwapProtocol', () => {
  let WXDC_xUSDT_V2: Pair
  let XSP_WXDC_V2: Pair
  let WXDC_USDC_V3: Pool
  let WXDC_USDC_V3_LOW_FEE: Pool
  let USDC_CGO_V3: Pool

  before(async () => {
    ;({ WXDC_xUSDT_V2, XSP_WXDC_V2, WXDC_USDC_V3, WXDC_USDC_V3_LOW_FEE, USDC_CGO_V3 } = await getXSwapPools(FORK_BLOCK))
  })

  describe('v2', () => {
    it('encodes a single exactInput XDC->xUSDT swap', async () => {
      const inputEther = utils.parseEther('1').toString()
      const trade = new V2Trade(
        new RouteV2([WXDC_xUSDT_V2], XDC, xUSDT),
        CurrencyAmount.fromRawAmount(XDC, inputEther),
    ...
    it('encodes a single exactInput XSP->XDC swap, with a fee', async () => {
      const inputEther = utils.parseEther('1').toString()
      const trade = new V2Trade(
        new RouteV2([XSP_WXDC_V2], XSP, XDC),
        CurrencyAmount.fromRawAmount(XSP, inputEther),
        TradeType.EXACT_INPUT
      )
    ...
    it('encodes an exactInput XSP->XDC->xUSDT swap', async () => {
      const inputEther = utils.parseEther('1').toString()
      const trade = new V2Trade(
        new RouteV2([XSP_WXDC_V2, WXDC_xUSDT_V2], XSP, xUSDT),
        CurrencyAmount.fromRawAmount(XSP, inputEther),
        TradeType.EXACT_INPUT
      )
      const opts = swapOptions({})
    ...
    //     const inputEther = utils.parseEther('1').toString()
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2, USDC_DAI_V2], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const feeOptions: FeeOptions = { fee: new Percent(5, 100), recipient: TEST_FEE_RECIPIENT_ADDRESS }
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2], xUSDT, XDC),
    //       CurrencyAmount.fromRawAmount(xUSDT, inputUSDC),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    //     const methodParameters = SwapRouter.swapERC20CallParameters(buildTrade([trade]), opts)
    ...
    //       TradeType.EXACT_INPUT
    //     )
    //     const permit = makePermit(xUSDT.address, inputUSDC, undefined, FORGE_ROUTER_ADDRESS)
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([USDC_DAI_V2, WETH_USDC_V2], DAI, XDC),
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2], XDC, xUSDT),
    //       CurrencyAmount.fromRawAmount(xUSDT, outputUSDC),
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2], XDC, xUSDT),
    //       CurrencyAmount.fromRawAmount(xUSDT, adjustedOutputUSDC),
    //       TradeType.EXACT_OUTPUT
    //     )
    //     const feeOptions: FeeOptions = { fee: new Percent(5, 100), recipient: TEST_FEE_RECIPIENT_ADDRESS }
    //     const opts = swapOptions({ fee: feeOptions })
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2], xUSDT, XDC),
    //       CurrencyAmount.fromRawAmount(XDC, outputUSDC),
    //       TradeType.EXACT_OUTPUT
    //     )
    ...
    //     const trade = new V2Trade(
    //       new RouteV2([WETH_USDC_V2], xUSDT, XDC),
    //       CurrencyAmount.fromRawAmount(XDC, outputETH),
    //       TradeType.EXACT_OUTPUT
    //     )
    //     const opts = swapOptions({})
    //     const methodParameters = SwapRouter.swapERC20CallParameters(buildTrade([trade]), opts)
    ...
    //       new RouteV3([WETH_USDC_V3], XDC, xUSDT),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const inputUSDC = utils.parseUnits('1000', 6).toString()
    //     const trade = await V3Trade.fromRoute(
    //       new RouteV3([WETH_USDC_V3], xUSDT, XDC),
    //       CurrencyAmount.fromRawAmount(xUSDT, inputUSDC),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const inputEther = utils.parseEther('1').toString()
    //     const trade = await V3Trade.fromRoute(
    //       new RouteV3([WETH_USDC_V3, USDC_DAI_V3], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const trade = await V3Trade.fromRoute(
    //       new RouteV3([WETH_USDC_V3], xUSDT, XDC),
    //       CurrencyAmount.fromRawAmount(XDC, outputEther),
    //       TradeType.EXACT_OUTPUT
    //     )
    //     const opts = swapOptions({})
    //     const methodParameters = SwapRouter.swapERC20CallParameters(buildTrade([trade]), opts)
    ...
    //     const outputEther = utils.parseEther('1').toString()
    //     const trade = await V3Trade.fromRoute(
    //       new RouteV3([USDC_DAI_V3, WETH_USDC_V3], DAI, XDC),
    //       CurrencyAmount.fromRawAmount(XDC, outputEther),
    //       TradeType.EXACT_OUTPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const trade = await V3Trade.fromRoute(
    //       new RouteV3([USDC_DAI_V3, WETH_USDC_V3], DAI, XDC),
    //       CurrencyAmount.fromRawAmount(XDC, adjustedOutputEther),
    //       TradeType.EXACT_OUTPUT
    //     )
    //     const feeOptions: FeeOptions = { fee: new Percent(5, 100), recipient: TEST_FEE_RECIPIENT_ADDRESS }
    //     const opts = swapOptions({ fee: feeOptions })
    ...
    //       new MixedRouteSDK([WETH_USDC_V3, USDC_DAI_V2], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const inputEther = utils.parseEther('1').toString()
    //     const trade = await MixedRouteTrade.fromRoute(
    //       new MixedRouteSDK([WETH_USDC_V2, USDC_DAI_V3], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const inputEther = utils.parseEther('1').toString()
    //     const trade = await MixedRouteTrade.fromRoute(
    //       new MixedRouteSDK([WETH_USDC_V2, USDC_DAI_V2], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const inputEther = utils.parseEther('1').toString()
    //     const trade = await MixedRouteTrade.fromRoute(
    //       new MixedRouteSDK([WETH_USDC_V3, USDC_DAI_V3], XDC, DAI),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //     const trade = await MixedRouteTrade.fromRoute(
    //       new MixedRouteSDK([USDC_DAI_V2, WETH_USDC_V3], DAI, XDC),
    //       CurrencyAmount.fromRawAmount(DAI, inputDai),
    //       TradeType.EXACT_INPUT
    //     )
    //     const opts = swapOptions({})
    ...
    //       new RouteV3([WETH_USDC_V3_LOW_FEE], XDC, xUSDT),
    //       CurrencyAmount.fromRawAmount(XDC, inputEther),
    //       TradeType.EXACT_INPUT
    //     )
    //
    ...
    //       const [tokenIn, tokenOut] = [DAI, xUSDT]
    ...
    //       const trade = new V2Trade(new RouteV2([USDC_DAI_V2], DAI, xUSDT), rawInputAmount, tradeType)
    //
    //       const classicQuote: PartialClassicQuote = {
    //         tokenIn: DAI.address,
    //         tokenOut: xUSDT.address,
    //         tradeType,
    //         route: [
    //           [
    //             mockV2PoolInRoute(
    ...
    //       const [tokenIn, tokenOut] = [WXDC, xUSDT]
    //       const inputAmount = ethers.utils
    ...
    //
    //       const opts = swapOptions({})
    //       const trade = await V3Trade.fromRoute(new RouteV3([WETH_USDC_V3], WXDC, xUSDT), rawInputAmount, tradeType)
    //
    //       const classicQuote: PartialClassicQuote = {
    //         tokenIn: WXDC.address,
    //         tokenOut: xUSDT.address,
    ...
    //       const [tokenIn, tokenOut] = [Ether.onChain(50), xUSDT]
    //       const inputAmount = ethers.utils
    //         .parseUnits('1', getAmountToken(tokenIn, tokenOut, tradeType).decimals)
    //         .toString()
    //       const rawInputAmount = getAmount(tokenIn, tokenOut, inputAmount, tradeType)
    //
    //       const opts = swapOptions({})
    //       const trade = new V2Trade(new RouteV2([WETH_USDC_V2], Ether.onChain(50), xUSDT), rawInputAmount, tradeType)
    //
    //       const classicQuote: PartialClassicQuote = {
    //         tokenIn: ETH_ADDRESS,
    //         tokenOut: xUSDT.address,
    ...
    //       const opts = swapOptions({})
    //       const trade = await V3Trade.fromRoute(
    //         new RouteV3([WETH_USDC_V3], Ether.onChain(50), xUSDT),
    //         rawInputAmount,
    //         tradeType
    //       )
    //
    ...
    //       const [tokenIn, tokenOut] = [xUSDT, Ether.onChain(50)]
    //       const inputAmount = ethers.utils
    //         .parseUnits('1', getAmountToken(tokenIn, tokenOut, tradeType).decimals)
    //         .toString()
    //       const rawInputAmount = getAmount(tokenIn, tokenOut, inputAmount, tradeType)
    ...
    //       const [tokenIn, tokenOut] = [DAI, WXDC]
    //       const inputAmount = ethers.utils
    //         .parseUnits('1', getAmountToken(tokenIn, tokenOut, tradeType).decimals)
    //         .toString()
    //       const rawInputAmount = getAmount(tokenIn, tokenOut, inputAmount, tradeType)
    ...
    //         tradeType,
    //         route: [
    //           [mockV3PoolInRoute(WETH_USDC_V3, WXDC, xUSDT, splitRouteInputAmounts[0], splitRouteOutputAmounts[0])],
    //           [
    //             mockV3PoolInRoute(
    ...
    //         [
    //           {
    //             ...mockV2PoolInRoute(USDC_DAI_V2, DAI, xUSDT, '1000', '1000'),
    //             tokenIn: undefined,
    //           },
    //         ],
    //       ],
